...
def exists(val):# hàm này sẽ trả về chính nó nếu nó tồn tại
    return val is not None

# identity sẽ trả về chính giá trị được đưa vào 
def identity(t, *args, **kwargs):
    return t

# hàm first trả về phần tử đầu tiên trong array  
def first(arr, d = None):
    if len(arr) == 0:
        return d
    return arr[0]

# Xây dựng hàm maybe là một decoratoe kiểm tra xem giá trị đầu vào x có tồn tại 
# hay không 
def maybe(fn):
    # hàm @wrap được sử dụng để định nghĩa decorator có chức năng để bảo toàn 
    # metadata của hàm fn 
    @wraps(fn)
    # hàm inner nhận đầu vào là tensor x 
    def inner(x, *args, **kwargs):
        if not exists(x): # nếu ko tồn tại 
            # trả về x ngay lập tức mà không cần gọi hàm fn
            return x
        # nếu x tồn tại gọi hàm fn với x và bất kỳ đối số nào được cung cấp 
        return fn(x, *args, **kwargs)
    return inner

# Xây dựng hàm default 
#  kiểm tra xem giá trị val có tồn tại hay không. 
# Nếu val tồn tại, nó sẽ trả về val
def default(val, d):
    if exists(val):
        return val
    # NẾU KHÔNG NÓ sẻ kiểm tra xem d có phải là một hàm gọi được hay không 
    # d là callable, nó sẽ gọi d() và trả về kết quả; nếu không, 
    # nó sẽ trả về d như là một giá trị mặc định.
    return d() if callable(d) else d
    

# Xây dựng hàm cast tuple hàm này sẽ được sử dụng để dịnh nghĩa 
# tuple cho một tensor 
def cast_tuple(val, length = None, validate = True):
    # nếu như val là một danh sách 
    if isinstance(val, list):
        # chuyển val thành 1 tuple 
        val = tuple(val)

    # trường hợp val ko phảo list 
    # nếu như nó là 1 tuple trả về chính nó 
    # còn không thì tạo một một tuple mới với val được nhân length lần 
    # hoặc 1 lần nếu length không được cung cấp  
    out = val if isinstance(val, tuple) else ((val,) * default(length, 1))

    # # Nếu length được cung cấp và validate là True
    if exists(length) and validate:
        # kiểm tra xem độ dài của out có bằng length không
        assert len(out) == length

    #  # Trả về tuple đã được chuyển đổi hoặc xác nhận
    return out
